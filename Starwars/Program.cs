...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Planet> tempPlanetNames1 = planets.Where(p => (p.Name.StartsWith("M"))).ToList();
            List<Planet> tempPlanetNames2 = planets.Where(p => (p.Name.Contains("y"))).ToList();
            List<Planet> tempPlanetNames3 = planets.Where(p => (p.Name.Count() < 15 && p.Name.Count() > 9)).ToList();
            List<Planet> tempPlanetNames4 = planets.Where(p => (p.Name[1].Equals("a") && p.Name.EndsWith("e"))).ToList();
            List<Planet> tempPlanetNames5 = planets.Where(p => (p.RotationPeriod > 40)).OrderBy(p => p.RotationPeriod).ToList();
            List<Planet> tempPlanetNames6 = planets.Where(p => (p.RotationPeriod > 10 && p.RotationPeriod < 20)).ToList();
            List<Planet> tempPlanetNames7 = planets.Where(p => (p.RotationPeriod > 30)).OrderBy(p => p.Name).ThenBy(p => p.Name).ToList();
            List<Planet> tempPlanetNames8 = planets.Where(p => ((p.RotationPeriod < 30 || p.SurfaceWater > 50) && p.Name.Contains("ba"))).ToList();
            List<Planet> tempPlanetNames9 = planets.Where(p => (p.SurfaceWater > 0)).OrderByDescending(p => p.SurfaceWater).ToList();
            List<Planet> tempPlanetNames10 = planets.Where(p => ((p.Diameter > 0) && (p.Population > 0))).OrderBy(p => (4 * 3.14 * ((p.Diameter / 2) * (p.Diameter / 2)) / p.Population)).ToList();
            List<Planet> tempPlanetNames11 = planets.Except(planets.Where(p => p.RotationPeriod > 0)).ToList();
            List<Planet> tempPlanetNames12 = planets.Where(p => (p.Name.StartsWith("A") || p.Name.EndsWith("s"))).Union(planets.Where(p => p.Terrain != null && p.Terrain.Contains("rainforests"))).Distinct().ToList();
            List<Planet> tempPlanetNames13 = planets.Where(p => p.Terrain != null && p.Terrain.Contains("deserts")).ToList();
            List<Planet> tempPlanetNames14 = planets.Where(p => p.Terrain != null && p.Terrain.Contains("swamps")).OrderBy(p => p.RotationPeriod).ThenBy(p => p.Name).ToList();
            List<Planet> tempPlanetNames15 = planets.Where(p => Regex.IsMatch(p.Name, @"([aeiou])\1")).ToList();
            List<Planet> tempPlanetNames16 = planets.Where(p => Regex.IsMatch(p.Name, @"([klrn])\1")).OrderByDescending(p => p.Name).ToList();
            Console.ReadKey();
            Console.WriteLine("Opgave: 1");
            for (int i = 0; i < tempPlanetNames16.Count; i++)
            {
                Console.WriteLine(tempPlanetNames16[i].Name);
            }
            Console.ReadKey();
        }
